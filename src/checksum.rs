use reqwest::Client;
use url::Url;
use std::{fs::File, io::{self, BufReader}, io::{Error, BufRead}, path::Path};
...
pub async fn verify(redirect_file_path: &Path, unpacked_file_path: &Path) -> Result<bool, std::io::Error> {
